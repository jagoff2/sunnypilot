"""Diffusion-based policy head producing planner-compatible outputs."""
...
from .modules import MLPHead, sinusoidal_time_embedding
...
class PolicyConditionEncoder(nn.Module):
  def __init__(self, embed_dim: int) -> None:
    ...
    self.hidden_proj = nn.Linear(ModelConstants.FEATURE_LEN, embed_dim)
    self.final_proj = nn.Linear(embed_dim * 3, embed_dim)
  ...
  def forward(self, features_buffer: torch.Tensor, desire: torch.Tensor,
    ...
    hist_vec = hist[:, -1]
    desire_vec = torch.relu(self.desire_proj(torch.cat([desire, traffic_convention], dim=-1)))
    hidden_vec = torch.relu(self.hidden_proj(hidden_state))
    cond = torch.cat([hist_vec, desire_vec, hidden_vec], dim=-1)
...
class DiffusionPolicyModel(nn.Module):
  def __init__(self, plan_dim: int = ModelConstants.PLAN_WIDTH, embed_dim: int = 256,
               diffusion_steps: int = 20) -> None:
    super().__init__()
    self.plan_dim = plan_dim
    self.condition_encoder = PolicyConditionEncoder(embed_dim)
    self.denoiser = PolicyDenoiser(plan_dim, embed_dim)
    self.schedule = DiffusionSchedule(timesteps=diffusion_steps)
    self.diffusion = DiffusionProcess(self.schedule)
    self.desire_head = MLPHead(embed_dim, ModelConstants.DESIRE_PRED_WIDTH)

  def forward(self, features_buffer: torch.Tensor, desire: torch.Tensor,
              traffic_convention: torch.Tensor, hidden_state: torch.Tensor) -> Dict[str, torch.Tensor]:
    conditioning = self.condition_encoder(features_buffer, desire, traffic_convention, hidden_state)
    plan = self.sample(conditioning)
    desire_state = torch.softmax(self.desire_head(conditioning), dim=-1)
    return {
      "plan": plan.unsqueeze(1),
      "plan_prob": torch.ones(plan.shape[0], ModelConstants.PLAN_MHP_SELECTION, device=plan.device),
  ...
  def loss(self, target_plan: torch.Tensor, features_buffer: torch.Tensor, desire: torch.Tensor,
           traffic_convention: torch.Tensor, hidden_state: torch.Tensor) -> torch.Tensor:
    conditioning = self.condition_encoder(features_buffer, desire, traffic_convention, hidden_state)
    batch_size = target_plan.shape[0]
    timesteps = torch.randint(0, self.schedule.timesteps, (batch_size,), device=target_plan.device)
    loss, _ = self.diffusion.p_losses(self.denoiser, target_plan, conditioning, timesteps)
    return loss
