"""Export the trained vision model to ONNX and tinygrad formats."""
...
class VisionExportWrapper(torch.nn.Module):
  ...
  def forward(self, img: torch.Tensor, big_img: torch.Tensor, desire: torch.Tensor,
              traffic_convention: torch.Tensor, features_buffer: torch.Tensor) -> torch.Tensor:
    outputs = self.model(img, big_img, desire=desire, traffic_convention=traffic_convention,
...
def main() -> None:
  ...
  dummy_inputs = {
    "img": torch.randn(1, 12, 128, 256, device=device),
    "big_img": torch.randn(1, 12, 256, 512, device=device),
    "desire": torch.zeros(1, 8, device=device),
    "traffic_convention": torch.zeros(1, 2, device=device),
    "features_buffer": torch.zeros(1, metadata.input_shapes["features_buffer"][1],
                                    metadata.input_shapes["features_buffer"][2], device=device),
  }
