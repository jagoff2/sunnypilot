...
class ZarrShardWriter:
  ...
  def _init_arrays(self) -> None:
    for name, shape in {**self.schema.vision_inputs, **self.schema.vision_targets, **self.schema.policy_targets}.items():
      array_shape = (0,) + shape
      ...
      if name in self._root:
        ...
        continue
      self._arrays[name] = self._root.create(name, shape=array_shape, chunks=chunks, dtype=dtype, compressor=zarr.Blosc(cname="zstd", clevel=3, shuffle=zarr.Blosc.SHUFFLE))
  ...
  def append(self, sample: MutableMapping[str, np.ndarray]) -> None:
    for key, array in self._arrays.items():
      ...
      array.append(np.asarray(value)[None, ...])
...
class CarlaZarrDataset(torch.utils.data.Dataset):
  ...
  def __init__(self, path: Path, schema: DatasetSchema = DEFAULT_SCHEMA, augmentations: Optional[Iterable[str]] = None,
  ...
  def __getitem__(self, index: int) -> Dict[str, torch.Tensor]:
    ...
    for name in self.schema.vision_inputs:
      ...
      sample[name] = torch.from_numpy(arr.astype(np.float32))
    for name in {**self.schema.vision_targets, **self.schema.policy_targets}:
      arr = np.asarray(self._arrays[name][index])
      sample[name] = torch.from_numpy(arr.astype(np.float32))
    return sample
