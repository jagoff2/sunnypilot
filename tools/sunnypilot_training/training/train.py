"""Training entry point for the diffusion-based sunnypilot models."""
...
def compute_vision_loss(outputs: Dict[str, torch.Tensor], batch: Dict[str, torch.Tensor]) -> torch.Tensor:
  ...
  losses.append(F.binary_cross_entropy(outputs["lane_lines_prob"], batch["lane_lines_prob"]))
  losses.append(F.binary_cross_entropy(outputs["lead_prob"], batch["lead_prob"]))
  losses.append(F.mse_loss(outputs["meta"], batch["meta"]))
  # Desire prediction: treat targets as categorical distribution
  target = torch.argmax(batch["desire_pred"], dim=-1)
  losses.append(F.cross_entropy(outputs["desire_pred"], target))
  losses.append(F.mse_loss(outputs["hidden_state"], batch["hidden_state"]))
  return sum(losses)


def save_checkpoint(output_dir: Path, epoch: int, vision: DiffusionVisionModel, policy: DiffusionPolicyModel,
...
def run_epoch(loader: DataLoader, vision: DiffusionVisionModel, policy: DiffusionPolicyModel,
              optimizer: torch.optim.Optimizer, scaler: torch.cuda.amp.GradScaler,
              device: torch.device, train: bool, cfg: DictConfig) -> Dict[str, float]:
  if train:
    vision.train()
    policy.train()
  else:
  ...
  for step, batch in enumerate(loader):
    ...
    with torch.cuda.amp.autocast(enabled=(device.type == "cuda" and cfg.mixed_precision)):
      vision_outputs = vision(batch["img"], batch["big_img"], desire=batch["desire"],
                              traffic_convention=batch["traffic_convention"],
      ...
      target_plan = batch["plan"].squeeze(1)
      conditioning = policy.condition_encoder(features_buffer, batch["desire"],
                                              batch["traffic_convention"], vision_outputs["hidden_state"])
      p_loss = policy.loss(target_plan, features_buffer, batch["desire"],
                           batch["traffic_convention"], vision_outputs["hidden_state"]) \
        + F.mse_loss(policy.sample(conditioning), target_plan)
      loss = v_loss + p_loss
...
def main(cfg: DictConfig) -> None:
  ...
  optimizer = torch.optim.AdamW(list(vision.parameters()) + list(policy.parameters()), lr=cfg.learning_rate,
