"""Numerical regression against the captured working c3x controller and NNLC.

The frozen fixture was generated by executing the unmodified captured control,
PID, extension, NNLC, and sigmoid code with these deterministic observations.
Only the vehicle observation/conversion boundary and Params storage are faked;
the production PID, NNLC model, sigmoid map, and capnp logging run in the tests.
"""
import ast
import hashlib
import json
...
class TestC3XTorqueController(unittest.TestCase):
  def test_matches_captured_full_controller_trace(self):
    reference = json.loads(FIXTURE.read_text())
    for enabled in (False, True):
      with self.subTest(nnlc=enabled):
        samples = run_trace(build_controller(enabled))
        expected = reference["nnlc_on" if enabled else "nnlc_off"]
        self.assertEqual([(s["case"], s["frame"]) for s in samples], [(s["case"], s["frame"]) for s in expected])
        np.testing.assert_allclose([s["values"] for s in samples], [s["values"] for s in expected], rtol=1e-6, atol=1e-7)
