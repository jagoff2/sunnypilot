...
class SigmoidMapSolution:
  ...
  def params_at_speed(self, speed: float) -> tuple[float, float] | None:
    if not self.slices:
      return None
    ...
    for low, high in zip(self.slices[:-1], self.slices[1:], strict=True):
...
class SigmoidMapTuner:
  ...
  def _speed_index(self, speed: float) -> int | None:
    idx = np.digitize(speed, self._speed_edges) - 1
    if 0 <= idx < len(self._speed_centers):
      return idx
    return None

  def _lat_index(self, lat_accel: float) -> int | None:
  ...
  def _slice_has_coverage(self, speed_idx: int) -> bool:
    ...
    neg_high = False
    mid = False
    for (idx_speed, _idx_lat), value in self._bins.items():
  ...
  def _solve(self) -> SigmoidMapSolution | None:
  ...
  def _fit_logistic(self, torque_space: np.ndarray, torques: np.ndarray) -> tuple[float | None, float | None]:
  ...
  def _torque_space_from_torque(self, torque, slope: float, intercept: float) -> float | None:
    steer_max = self.lac_torque.steer_max
  ...
  def _update_torque_params(self) -> None:
    ...
    self.lac_torque.update_torque_parameters(lat_accel_factor, lat_accel_offset, friction)

  def _estimate_latacc_factor(self) -> float | None:
    slopes = []
  ...
  def _write_solution(self) -> None:
    ...
    for directory in self._writable_storage_dirs:
    ...
    for directory in self._storage_dirs:
      ...
      try:
        with open(path, encoding="utf-8") as f:
          data = json.load(f)
        if not isinstance(data, dict) or not isinstance(data.get("slices"), list) or not data["slices"]:
          continue
        valid = all(isinstance(item, dict) and all(
          isinstance(item.get(key), (int, float)) and not isinstance(item[key], bool) and math.isfinite(item[key])
          for key in ("speed", "slope", "intercept")) and item["slope"] > 0.0 for item in data["slices"])
        if not valid:
          continue
  ...
  def _candidate_storage_dirs(self) -> list[str]:
    # Prefer a vehicle's existing map. The packaged fallback is the frozen
    # Kona map captured from the working c3x, used only when no local map exists.
    return ["/data/params/d_tmp/sigmoid_maps", os.path.join(os.path.dirname(__file__), "sigmoid_maps")]
